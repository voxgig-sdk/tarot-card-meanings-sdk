# tarot-card-meanings-cli

AQL-driven command-line client **and** interactive REPL for the TarotCardMeanings
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/tarot-card-meanings-cli)
make build

# 2. See usage (words, entities, env vars)
./tarot-card-meanings-cli --help
...
./tarot-card-meanings-cli list card
./tarot-card-meanings-cli load 1 card            # {id:1} shorthand
./tarot-card-meanings-cli load '{id:1}' card       # explicit match map

# 5. Override the API base URL for a single call
TAROT_CARD_MEANINGS_BASE=https://api.example.com ./tarot-card-meanings-cli list card

# 6. No arguments -> interactive REPL
./tarot-card-meanings-cli
tarot-card-meanings> list card
tarot-card-meanings> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/tarot-card-meanings-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/tarot-card-meanings-cli list card
   ```
...
```sh
./tarot-card-meanings-cli load 1 card          # scalar shorthand for {id:1}
./tarot-card-meanings-cli load '{id:1}' card     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `tarot-card-meanings>`). Each line is
evaluated as its own AQL expression:

```text
$ ./tarot-card-meanings-cli
tarot-card-meanings> list card
tarot-card-meanings> /help
tarot-card-meanings> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `card`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/tarot-card-meanings-cli`. |
...
(`./tarot-card-meanings-cli <expr>`) and interactively (the REPL), and expressions compose the
