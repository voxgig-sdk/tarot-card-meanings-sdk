// Command TarotCardMeanings-cli is a CLI + REPL for the TarotCardMeanings SDK,
...
const prompt = "tarot-card-meanings"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "card"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TAROT_CARD_MEANINGS_APIKEY (api key), TAROT_CARD_MEANINGS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TAROT_CARD_MEANINGS_APIKEY carries the API key and
	// TAROT_CARD_MEANINGS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TAROT_CARD_MEANINGS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TAROT_CARD_MEANINGS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTarotCardMeaningsSDK(opts)
