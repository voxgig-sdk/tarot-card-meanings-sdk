...
//     (\`new TarotCardMeaningsSDK(...)\` / \`TarotCardMeaningsSDK.test(...)\` become
//     \`TarotCardMeaningsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'TarotCardMeaningsSDK'
...
// rewrite real client construction (\`new TarotCardMeaningsSDK(...)\` and any
// \`TarotCardMeaningsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TarotCardMeaningsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TarotCardMeaningsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
