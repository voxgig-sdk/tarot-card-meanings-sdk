...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TAROTCARDMEANINGS_TEST_LIVE") == "TRUE" ||
		os.Getenv("TAROTCARDMEANINGS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TAROTCARDMEANINGS_TEST_EXPLAIN"); explain != "" {
