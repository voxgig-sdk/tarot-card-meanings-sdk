...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TAROT_CARD_MEANINGS_TEST_LIVE") == "TRUE" ||
		os.Getenv("TAROT_CARD_MEANINGS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TAROT_CARD_MEANINGS_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.TarotCardMeaningsSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.TarotCardMeaningsSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.TarotCardMeaningsError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.TarotCardMeaningsSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.TarotCardMeaningsError{Msg: msg, Code: code}
}
