...
import (
	...
	omni "github.com/voxgig-sdk/tarot-card-meanings-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TarotCardMeaningsError); is {
				out := map[string]any{
					"name":    "TarotCardMeaningsError",
...
func omniCtx(arg any, client *sdk.TarotCardMeaningsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TarotCardMeaningsSDK); is {
			client = live
