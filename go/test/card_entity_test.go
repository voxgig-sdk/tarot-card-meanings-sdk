...
func cardBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TAROT_CARD_MEANINGS_TEST_CARD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TAROT_CARD_MEANINGS_TEST_CARD_ENTID"])
	...
	if env["TAROT_CARD_MEANINGS_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewTarotCardMeaningsSDK(core.ToMapAny(mergedOpts))
	}
