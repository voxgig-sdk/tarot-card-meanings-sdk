# tarot-card-meanings-mcp

[MCP](https://modelcontextprotocol.io) server exposing the TarotCardMeanings SDK as
two agent tools — `tarot-card-meanings_list` and `tarot-card-meanings_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/tarot-card-meanings-mcp)
make build

# 2. Provide credentials via the environment
export TAROT_CARD_MEANINGS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user tarot-card-meanings \
  -- /absolute/path/to/tarot-card-meanings-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./tarot-card-meanings-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// tarot-card-meanings_list: first page of records
{ "entity": "card" }
{ "entity": "card", "query": { } }

// tarot-card-meanings_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/tarot-card-meanings-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user tarot-card-meanings \
     -- "$PWD"/dist/*/tarot-card-meanings-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `tarot-card-meanings_list` and `tarot-card-meanings_load` tools now appear
   in new sessions. Ask the agent to *"list card using tarot-card-meanings"*
   and it calls `tarot-card-meanings_list` with `{"entity":"card"}`.
...
```sh
export TAROT_CARD_MEANINGS_APIKEY=sk_live_xxx            # API key
export TAROT_CARD_MEANINGS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `tarot-card-meanings_list` tool
...
### Call the `tarot-card-meanings_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `tarot-card-meanings_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `tarot-card-meanings_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TAROT_CARD_MEANINGS_APIKEY` | API key sent with every request. |
| `TAROT_CARD_MEANINGS_BASE` | Optional override of the API base URL. |
...
```sh
./tarot-card-meanings-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"tarot-card-meanings_load","arguments":{"entity":"card","query":{"id":1}}}}'
```
