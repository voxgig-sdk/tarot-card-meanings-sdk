...
//    only - a typed TarotCardMeaningsContext there would make every ctx
...
final class TarotCardMeaningsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TarotCardMeaningsContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new TarotCardMeaningsSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new TarotCardMeaningsResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new TarotCardMeaningsError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new TarotCardMeaningsResponse($clean['response']);
    ...
    public static function err_from_map($m): ?TarotCardMeaningsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TarotCardMeaningsError(is_string($code) ? $code : '', $msg);
    }
