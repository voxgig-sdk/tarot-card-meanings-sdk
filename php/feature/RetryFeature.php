...
// TarotCardMeanings SDK retry feature
...
class TarotCardMeaningsRetryFeature extends TarotCardMeaningsBaseFeature
{
    ...
    public function init(TarotCardMeaningsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TarotCardMeaningsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TarotCardMeaningsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TarotCardMeaningsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
