...
// TarotCardMeanings SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class TarotCardMeaningsTestFeature extends TarotCardMeaningsBaseFeature
{
    ...
    public function init(TarotCardMeaningsContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (TarotCardMeaningsContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = TarotCardMeaningsParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (TarotCardMeaningsContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(TarotCardMeaningsContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = TarotCardMeaningsParam::call($ctx, $k);
