...
from tarotcardmeanings_sdk.core.helpers import get_ctx_prop, to_map
...
class TarotCardMeaningsContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = TarotCardMeaningsControl()
        ...
        if isinstance(sp, TarotCardMeaningsSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, TarotCardMeaningsResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, TarotCardMeaningsResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return TarotCardMeaningsOperation({})
    ...
    def make_error(self, code, msg):
        return TarotCardMeaningsError(code, msg, self)
