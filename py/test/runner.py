# TarotCardMeanings SDK test runner
...
class TarotCardMeaningsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TarotCardMeaningsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TarotCardMeaningsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TarotCardMeaningsTestRunner.getenv("TAROT_CARD_MEANINGS_TEST_LIVE")
        override = TarotCardMeaningsTestRunner.getenv("TAROT_CARD_MEANINGS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TarotCardMeaningsTestRunner.getenv(key)
        ...
        explain = TarotCardMeaningsTestRunner.getenv("TAROT_CARD_MEANINGS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if TarotCardMeaningsTestRunner._test_control is not None:
            return TarotCardMeaningsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TarotCardMeaningsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TarotCardMeaningsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TarotCardMeaningsTestRunner.load_test_control()
...
def load_env_local():
    TarotCardMeaningsTestRunner.load_env_local()


def env_override(m):
    return TarotCardMeaningsTestRunner.env_override(m)


def entity_data(v):
    return TarotCardMeaningsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TarotCardMeaningsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TarotCardMeaningsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TarotCardMeaningsTestRunner.load_test_control()


def live_client_options():
    return TarotCardMeaningsTestRunner.live_client_options()


def live_delay_ms():
    return TarotCardMeaningsTestRunner.live_delay_ms()
