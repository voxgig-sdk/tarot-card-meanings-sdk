...
module TarotCardMeaningsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TarotCardMeaningsResponse.new({})
    ...
    if fetch_err
      ...
      response = TarotCardMeaningsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TarotCardMeaningsResponse.new(fetched)
    else
