...
class TarotCardMeaningsMakeError
{
    public static function call(?TarotCardMeaningsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TarotCardMeaningsContext([], null);
        }
        $op = $ctx->op ?? new TarotCardMeaningsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TarotCardMeaningsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TarotCardMeaningsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new TarotCardMeaningsError('', $msg, $ctx);
        ...
        if ($err instanceof TarotCardMeaningsError) {
