...
// TarotCardMeanings SDK feature test
//
...
class FtCtrl extends TarotCardMeaningsControl
{
...
class FtClient
{
    ...
    private TarotCardMeaningsUtility $utility;

    public function __construct(TarotCardMeaningsUtility $utility, array $options)
    {
    ...
    public function get_utility(): TarotCardMeaningsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TarotCardMeaningsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TarotCardMeaningsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TarotCardMeaningsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TarotCardMeaningsConfig::make_feature($name);
    ...
    private static function buildUrl(TarotCardMeaningsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TarotCardMeaningsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TarotCardMeaningsResponse($fetched);
        }
    ...
    private function fail(TarotCardMeaningsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TarotCardMeaningsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
