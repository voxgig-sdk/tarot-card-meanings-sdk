# TarotCardMeanings Ruby SDK



The Ruby SDK for the TarotCardMeanings API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
client = TarotCardMeaningsSDK.test(testopts, sdkopts)
```
...
raise a `TarotCardMeaningsError` (a `StandardError` subclass), so wrap
...
```
rb/
├── TarotCardMeanings_sdk.rb       -- Main SDK module
...
The main module (`TarotCardMeanings_sdk`) exports the SDK class
