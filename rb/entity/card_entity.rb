# TarotCardMeanings SDK Card entity
...
class CardEntity
  ...
  def data_set(args)
    if args
      @_data = TarotCardMeaningsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = TarotCardMeaningsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Card, Hash] the loaded Card; raises TarotCardMeaningsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = TarotCardMeaningsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Card>, Array] the matching Card items; raises TarotCardMeaningsError on failure
