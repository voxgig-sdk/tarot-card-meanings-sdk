...
require_relative 'TarotCardMeanings_types'
...
class TarotCardMeaningsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = TarotCardMeaningsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = TarotCardMeaningsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, TarotCardMeaningsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    TarotCardMeaningsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = TarotCardMeaningsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = TarotCardMeaningsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = TarotCardMeaningsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => TarotCardMeaningsError.new(
        "#{op}_allow",
        "TarotCardMeaningsSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue TarotCardMeaningsError => err
    ...
    if fetched.is_a?(Hash)
      status = TarotCardMeaningsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = TarotCardMeaningsError.new(
        "graphql_error", "TarotCardMeaningsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = TarotCardMeaningsSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
